...
class GenomeTest : public CPPUNIT_NS::TestFixture
{
	...
	protected:
			...
			protected:
				void random_impl(const uint32_t count, IOutputAdapter<shared_ptr<Genome<City>>> &output)
				{
					...
					for(uint32_t i = 0; i < count; i++)
					{
						...
						{
							CPPUNIT_ASSERT(numbers[m] >= 0);
							CPPUNIT_ASSERT(numbers[m] < N_CITIES);

							genome->set(m, create_new_city(numbers[m]));
						}

						CPPUNIT_ASSERT_EQUAL(genome->size(), N_CITIES);
		...
		void test_operators()
		{
			...
			{
				CPPUNIT_ASSERT(equals(reinterpret_cast<const Genome<City>*>(&(*population[i])),
				                      reinterpret_cast<const Genome<City>*>(&(*population[i + 1]))));
			}
